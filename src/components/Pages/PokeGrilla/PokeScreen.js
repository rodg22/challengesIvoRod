...
export const PokeScreen = () => {
  ...
  return (
    <>
    {
      ...
      <h1>{name ? name[0].toUpperCase() + name?.substring(1) : null}</h1>
      <h2>ID: {id}</h2>
      <h2>height: {height}</h2>
      <h2>weight: {weight}</h2>
      <div className="typosEnPokeScreen" style={{ position: "relative" }}>
      {types?.map(({ type: { name } }, index) => {
        ...
        <div>
          <button className="pokeScreenButtons" onClick={handleAbilityOpen}>Abilities:</button>
          {showAbility && (
            <ul>
        ...
        <div>
          <button className="pokeScreenButtons" onClick={handleStatsOpen}>Stats:</button>
          {showStats && (
            <ul>
              {stats?.map((stat, index) => {
                return   (
                  <li style={{color: 'black', textAlign: 'center'}} key={index}>
        ...
        <div>
          <button className="pokeScreenButtons" onClick={handleMovesOpen}>Moves:</button>
          {showMoves && (
            <ul>
